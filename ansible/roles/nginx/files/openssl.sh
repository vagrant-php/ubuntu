...
then
  echo
  openssl req -nodes -x509 -newkey rsa:2048 -keyout /etc/nginx/$2.$1.key -reqexts SAN -extensions SAN -out /etc/nginx/$2.$1.crt -days 3650 -subj "/C=CH/ST=Zurich/L=Zurich/CN=$1" -config <(cat /etc/ssl/openssl.cnf; printf "[SAN]\nsubjectAltName=DNS:$1,DNS:$2.$1")
  cp -f /etc/nginx/$2.$1.crt ${EXPORT_CERT_DIR}/.
  cp -f /etc/nginx/$2.$1.crt ${IMPORT_CERT_DIR}/.
  update-ca-certificates
  exit 0
fi
